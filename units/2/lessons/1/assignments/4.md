...
As we build the application we are to be introducing a new module called `logging`.  This allows us to track what our program is doing and makes an excellent tool for debugging code.  The first thing we need to do is to configure the logging module using the `logging.basicConfig` function.  Create a new file called *snippets.py* and add the following code:
...
The `filename` argument describes where we want the log to be saved.  The `level` argument sets the log level.  The logging module splits log messages into one of five severity levels:
...
Now let's start looking at how to save snippets. We are going to store the snippets in a CSV file using the `csv` module. So lets try to write a function which:
